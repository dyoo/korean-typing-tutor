...
/**
 * Creates the initial default MasteryState starting at Stage 1 Home Row (4 Jamos unlocked).
 */
  ...
  for (const item of selectionPool) {
    ...
    for (const j of jamos) {
      ...
      if (stats && stats.totalAttempts > 0) {
        ...
        if (acc < 0.95) {
          // Add continuous error-proportional weight: lower accuracy = higher weight
          jamoWeight += (0.95 - acc) * 8.0;
        }
      }
    }
