...
/**
 * Universal deck parser for remote URLs pointing to .apkg, .txt, .tsv, or .csv files.
 */
export async function parseDeckFromUrl(url: string): Promise<CustomDeck> {
  ...
  try {
    ...
    throw new Error(
      `Failed to fetch deck from URL. Please check network connection and ensure the server allows CORS requests. (${(error as Error).message})`,
      { cause: error },
    );
  }

  if (!response.ok) {
    throw new Error(`Server returned HTTP ${response.status} ${response.statusText} when downloading deck.`);
  }

  // Extract filename from URL or Content-Disposition
  const urlPath = new URL(trimmedUrl).pathname;
  let filename = urlPath.split('/').pop() || 'Remote Deck';
  try {
  ...
  const isApkg = filename.toLowerCase().endsWith('.apkg') || contentType.includes('application/zip') || contentType.includes('application/octet-stream');
  ...
  const deck = parseTextFlashcards(text, filename);
  if (deck.items.length === 0) {
    throw new Error(`No valid Korean flashcards found at URL "${trimmedUrl}".`);
  }
  return deck;
}
