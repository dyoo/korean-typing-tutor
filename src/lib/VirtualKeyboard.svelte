<script lang="ts">
  ...
  let isLeftShiftTarget = $derived(activeKeys.includes('left-shift') || activeKeys.includes('shift'));
  let isRightShiftTarget = $derived(activeKeys.includes('right-shift') || activeKeys.includes('shift'));
    ...
    <div class="flex items-center justify-center gap-1 sm:gap-1.5 w-full">
      ...
      {#each row as cap}
        ...
        >
          ...
          <div class="flex items-center justify-between w-full text-[9px] sm:text-[10px] leading-none font-mono">
