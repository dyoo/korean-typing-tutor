...
>
  <span class="font-bold text-gray-900 dark:text-gray-100">Cursor Color</span>

  <div class="relative">
    <button
      type="button"
      onclick={() => (isOpen = !isOpen)}
      class="flex items-center gap-2 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-800 dark:text-gray-200 font-medium rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:border-blue-600 cursor-pointer shadow-2xs"
    >
      <span class="w-3.5 h-3.5 rounded-full shrink-0 {currentOption.swatchClass}"></span>
    ...
    {#if isOpen}
      ...
      <div
        class="absolute right-0 top-full mt-1 z-[70] w-48 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg py-1 flex flex-col gap-0.5"
      >
        {#each OPTIONS as opt}
          ...
          <button
            ...
            class="flex items-center gap-2.5 px-3 py-2 text-sm font-medium text-left hover:bg-gray-100 dark:hover:bg-gray-700/80 transition-colors cursor-pointer {isSelected
              ? 'text-blue-600 dark:text-blue-400 font-bold bg-blue-50/50 dark:bg-blue-950/30'
              : 'text-gray-700 dark:text-gray-200'}"
          >
            <span class="w-3.5 h-3.5 rounded-full shrink-0 {opt.swatchClass}"></span>
